...
}), angular.module("openshiftCommonUI", []), hawtioPluginLoader.addModule("openshiftCommonUI"), angular.module("openshiftCommonUI").run([ "$templateCache", function($templateCache) {
"use strict";
...
$scope.isDialog = "true" === $scope.isDialog, $scope.createProject = function() {
...
cb ? cb(encodeURIComponent(data.metadata.name)) :$location.path("project/" + encodeURIComponent(data.metadata.name) + "/create");
}, function(result) {
...
$scope.alerts["error-creating-project"] = {
type:"error",
message:msg
};
}
...
} ]), angular.module("openshiftCommonUI").directive("deleteProject", [ "$uibModal", "$location", "$filter", "$q", "hashSizeFilter", "APIService", "DataService", "AlertMessageService", "Logger", function($uibModal, $location, $filter, $q, hashSizeFilter, APIService, DataService, AlertMessageService, Logger) {
return {
restrict:"E",
scope:{
projectName:"@",
...
scope.stayOnCurrentPage ? scope.alerts[alert.name] = alert.data :AlertMessageService.addAlert(alert);
...
var cb = $scope.redirectAction();
cb && cb(encodeURIComponent($scope.project.metadata.name));
}, function(result) {
$scope.disableInputs = !1, $scope.editableFields = editableFields($scope.project), $scope.alerts.update = {
type:"error",
...
step.onNextOrig = step.onNext, step.onPrevOrig = step.onPrev, step.onNext = onStepNext, step.onPrev = onStepPrev, step.fixedElement = !0, !angular.isUndefined(step.yOffset) || "right" !== step.placement && "left" !== step.placement || (step.yOffset = -45);
});
}
function onStepNext() {
